  ...
  return (
    <div className="animate__animated animate__fadeIn">
      ...
      <div className="section-hero">
        ...
        <p className="font-condensed text-gold/90 text-base sm:text-lg max-w-sm mx-auto">
          {data.heroSubtitulo}
        </p>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4">
        {data.items.map((b, idx) => (
          <div
            key={idx}
            className={`relative flex items-center gap-4 sm:gap-6 px-4 sm:px-6 py-6 sm:py-8
          ...
          >
            {/* Ícono */}
            {b.icono && (
              <div className="flex-shrink-0 w-14 h-14 sm:w-20 sm:h-20 rounded-2xl bg-gold/[0.07] flex items-center justify-center
                              transition-colors duration-300 group-hover:bg-gold/[0.13]">
                <img src={b.icono} alt={b.titulo} className="w-8 h-8 sm:w-12 sm:h-12 object-contain" />
              </div>
            )}

            {/* Texto */}
            <div className="flex-1 min-w-0">
              <h3 className="font-condensed font-bold text-gold tracking-[1px] sm:tracking-[2px] uppercase
                             text-base sm:text-lg leading-tight mb-2">
                {b.titulo}
              </h3>
            ...
            <span
              aria-hidden="true"
              className="hidden sm:block absolute right-5 font-condensed font-bold text-[5rem] leading-none
